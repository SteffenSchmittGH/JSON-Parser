...
void ADC_Joy_IntClock_Start(void) 
{
    ...
    ADC_Joy_IntClock_CLKEN |= ADC_Joy_IntClock_CLKEN_MASK;
	ADC_Joy_IntClock_CLKSTBY |= ADC_Joy_IntClock_CLKSTBY_MASK;
}
...
void ADC_Joy_IntClock_Stop(void) 
{
    ...
    ADC_Joy_IntClock_CLKEN &= (uint8)(~ADC_Joy_IntClock_CLKEN_MASK);
	ADC_Joy_IntClock_CLKSTBY &= (uint8)(~ADC_Joy_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_Joy_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ADC_Joy_IntClock_CLKEN & ADC_Joy_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ADC_Joy_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_Joy_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC_Joy_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_Joy_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_Joy_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_Joy_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ADC_Joy_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ADC_Joy_IntClock_GetSourceRegister();
    uint16 oldDivider = ADC_Joy_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_Joy_IntClock_CLKEN & ADC_Joy_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC_Joy_IntClock_DIV_PTR, clkDivider);
                ADC_Joy_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC_Joy_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC_Joy_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_Joy_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADC_Joy_IntClock_DIV_PTR, clkDivider);
				ADC_Joy_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
{
    return CY_GET_REG16(ADC_Joy_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_Joy_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_Joy_IntClock_MOD_SRC |= modeBitMask & (uint8)ADC_Joy_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_Joy_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_Joy_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_Joy_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_Joy_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_Joy_IntClock_MOD_SRC & (uint8)(ADC_Joy_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_Joy_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ADC_Joy_IntClock_GetDividerRegister();
    uint8 oldSrc = ADC_Joy_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_Joy_IntClock_MOD_SRC |= CYCLK_SSS;
        ADC_Joy_IntClock_MOD_SRC =
            (ADC_Joy_IntClock_MOD_SRC & (uint8)(~ADC_Joy_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_Joy_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_Joy_IntClock_MOD_SRC & ADC_Joy_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_Joy_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ADC_Joy_IntClock_PHASE = clkPhase & ADC_Joy_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_Joy_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ADC_Joy_IntClock_PHASE & ADC_Joy_IntClock_PHASE_MASK;
}
